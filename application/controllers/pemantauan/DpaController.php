<?php
	...
	class DpaController extends CI_Controller {
        ...
        public function dpa22($skpd = '')
		{
			...
			$body = $this->pantau_model->dpa22_pdf($skpd);
			$filename = 'DPA_22_'.$skpdFile.'.pdf';
			$this->public_model->_mpdf('',$header,$body,$mg['L'],$mg['R'],$mg['T'],$mg['B'],10,$orientation,1,true,'A4',$filename);
        }

        public function dpa22_excel($skpd = '')
		{
            $header = '';
            $skpdFile = str_replace('.', '_', $skpd);
			$data['table'] = $this->pantau_model->dpa22_pdf($skpd);
            $data['filename'] = 'DPA_22_'.$skpdFile.'.xls';
        ...
        public function dpa221($skpd = '',$kegiatan = '')
		{
			$margins = $_GET['margins'];
			$orientation = $_GET['orientation'];
			$mg = $this->PublicModel->getMargins($margins);
			...
			$body = $this->pantau_model->dpa221_pdf($skpd,$kegiatan);
			$filename = 'DPA_221_'.$kegFile.'.pdf';
			$this->public_model->_mpdf('',$header,$body,$mg['L'],$mg['R'],$mg['T'],$mg['B'],10,$orientation,1,true,'A4',$filename);
        }
        
        public function dpa221_excel($skpd = '',$kegiatan = '')
		{
            $header = '';
            $kegFile = str_replace('.', '_', $kegiatan);
            $data['table'] = $this->pantau_model->dpa221_pdf($skpd,$kegiatan);
            $data['filename'] = 'DPA_221_'.$kegFile.'.xls';
            $this->load->view('laporan/excel', $data);
			// $filename = 'DPA_221_'.$kegFile.'.pdf';
			// $this->public_model->_mpdf('',$header,$body,10,10,10,10,10,'P',1,true,'A4',$filename);
		}
		...
		public function get_combo_skpd() {
            $list = $this->public_model->get_combo_skpd();
	        echo $list;
        }
        
        public function get_combo_kegiatan() {
            $skpd = $_POST['skpd'];
            $list = $this->public_model->get_combo_kegiatan($skpd);
	        echo $list;
		}
