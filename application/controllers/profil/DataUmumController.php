<?php
	defined('BASEPATH') OR exit('No direct script access allowed');

	class DataUmumController extends CI_Controller {

		public function __construct(){
			parent::__construct();
			$this->load->model('setting/user_model', 'user_model');
			$this->load->model('admin/user_model', 'user_model2');
            $this->load->model('admin/opd_model', 'opd_model');
            $this->load->model('profil/DataUmumModel', 'umum_model');
            $this->load->model('PublicModel', 'publicmodel');
		}

		public function index(){
			$data['view'] = 'profil/dataUmum/dataUmumView';
			$this->load->view('template/layout', $data);
		}

        public function add_data_umum()
		{
            $data['view'] = 'profil/dataUmum/dataUmumAddView';
            $this->load->view('template/layout', $data);
        }

        public function edit_data_umum()
		{
            $ta = $_GET['thn'];
            $this->db->select('*');
            $this->db->where('tahun_anggaran',$ta);
            $this->db->from('ms_data_umum');
            $result = $this->db->get()->result_array();
            $data['dataumum'] = $result[0];
            $data['view'] = 'profil/dataUmum/dataUmumEditView';
            $this->load->view('template/layout', $data);
        }

        public function hapus_data_umum(){
            $id = $_GET['thn'];
			$this->db->delete('ms_data_umum', array('tahun_anggaran' => $id));
			$this->session->set_flashdata('msg', 'Data Berhasil Dihapus!');
			redirect(base_url('data-umum-daerah'));
		}
        
        public function insert_data_umum()
		{
            // cek database 
            $ta = $_POST['tahun_anggaran'];
            

            $this->db->select('*');
            $this->db->where('tahun_anggaran',$ta);
            $this->db->from('ms_data_umum');
            $result = $this->db->get()->result();
            if(count($result)==0){
                $nm_file_temp = $_POST['tahun_anggaran'].'_'.$_POST['nama_daerah'];
                
			    $search = array("/", " ", ":", "*", "?", "&#34;", "<", ">", "|");
			    $replace = array("_", "_", "_", "_", "_", "_", "_", "_", "_");
			    $fileTemp = str_replace($search, $replace, $nm_file_temp);

			    $folder			= 'assets/img/logo/'.$fileTemp.'/';
                
			
                if (!file_exists($folder))
                {
                    mkdir($folder); 
                }
                



			$files = $_FILES['logo_file'];  
            
            $config=array(  
	            'upload_path' => './'.$folder, 
	            'allowed_types' => '*',  
	            'max_size' => '200000',  
	            'max_width' => '200000',  
	            'max_height' => '200000'  
            );  
            $i = 0;
            $upload = 0;
            $convertFile = str_replace(' ', '_', $files['name']);
            $pathAndFile = $folder.$convertFile;
            
            
            if($convertFile <> ''){
                
                $_FILES['logo_file']['name'] = $convertFile; 
                $_FILES['logo_file']['type'] = $files['type'];  
                $_FILES['logo_file']['tmp_name'] = $files['tmp_name'];  
                $_FILES['logo_file']['error'] = $files['error'];  
                $_FILES['logo_file']['size'] = $files['size'];  
                $this->load->library('upload', $config);  
                if (file_exists($pathAndFile)) {
                	unlink($pathAndFile);
                }  
                $this->upload->do_upload('logo_file');
                $upload = 1;
         	}else{
         		$convertFile = '';
             }
             



                
                $insertData = array(
                    'tahun_anggaran' 		=> $this->input->post('tahun_anggaran'),
                    'nama_daerah' 		=> $this->input->post('nama_daerah'),
                    'ibukota' 		=> $this->input->post('ibukota'),
                    'kepala_daerah' 		=> $this->input->post('kepala_daerah'),
                    'tgl_dpa' 				=> $this->input->post('tgl_dpa'),
                    'tgl_dppa' 				=> $this->input->post('tgl_dppa'),
                    'logo_daerah' 			=> $pathAndFile,
                    'user_input' 			=> $this->session->userdata('name'),
                    'date_input' 			=> date('Y-m-d H:i:s'),
                );
                $dataInsert = $this->security->xss_clean($insertData);
                $result = $this->db->insert('ms_data_umum',$dataInsert);
                $data['view'] = 'profil/dataUmum/dataUmumView';
			    $this->load->view('template/layout', $data);
            }else{
                $data['dataumum'] = $_POST;
                $data['view'] = 'profil/dataUmum/dataUmumAddView';
                $this->load->view('template/layout', $data);
            }
        }
        
        public function update_data_umum()
		{
            // cek database 
            $ta = $_POST['tahun_anggaran'];
            
            
                $nm_file_temp = $_POST['tahun_anggaran'].'_'.$_POST['nama_daerah'];
                
			    $search = array("/", " ", ":", "*", "?", "&#34;", "<", ">", "|");
			    $replace = array("_", "_", "_", "_", "_", "_", "_", "_", "_");
			    $fileTemp = str_replace($search, $replace, $nm_file_temp);

			    $folder			= 'assets/img/logo/'.$fileTemp.'/';
                
			
                if (!file_exists($folder))
                {
                    mkdir($folder); 
                }
                



			$files = $_FILES['logo_file'];  
            
            $config=array(  
	            'upload_path' => './'.$folder, 
	            'allowed_types' => '*',  
	            'max_size' => '200000',  
	            'max_width' => '200000',  
	            'max_height' => '200000'  
            );  
            $i = 0;
            $upload = 0;
            $convertFile = str_replace(' ', '_', $files['name']);
            $pathAndFile = $folder.$convertFile;
            
            
            if($convertFile <> ''){
                
                $_FILES['logo_file']['name'] = $convertFile; 
                $_FILES['logo_file']['type'] = $files['type'];  
                $_FILES['logo_file']['tmp_name'] = $files['tmp_name'];  
                $_FILES['logo_file']['error'] = $files['error'];  
                $_FILES['logo_file']['size'] = $files['size'];  
                $this->load->library('upload', $config);  
                if (file_exists($pathAndFile)) {
                	unlink($pathAndFile);
                }  
                $this->upload->do_upload('logo_file');
                $upload = 1;
         	}else{
                 $convertFile = '';
                 $pathAndFile = $this->input->post('logo_daerah');
             }

                
                $updateData = array(
                    'nama_daerah' 		=> $this->input->post('nama_daerah'),
                    'ibukota' 		=> $this->input->post('ibukota'),
                    'kepala_daerah' 		=> $this->input->post('kepala_daerah'),
                    'tgl_dpa' 				=> $this->input->post('tgl_dpa'),
                    'tgl_dppa' 				=> $this->input->post('tgl_dppa'),
                    'logo_daerah' 			=> $pathAndFile,
                    'user_update' 			=> $this->session->userdata('name'),
                    'date_update' 			=> date('Y-m-d H:i:s'),
                );

                $dataUpdate = $this->security->xss_clean($updateData);
                $this->db->where('tahun_anggaran', $ta);
                $result = $this->db->update('ms_data_umum', $dataUpdate);
                $data['view'] = 'profil/dataUmum/dataUmumView';
			    $this->load->view('template/layout', $data);
		}

		public function cek_user()
		{
			$user = $_POST['username'];
			$cekUser = $this->db->get_where('ms_user', array('username' => $user))->result();
			if (count($cekUser) == 1) {
				$data['sts']= 'ada';
			}else{
				$data['sts']= 'tdk';
			}
			echo json_encode($data);
		}

		public function get_data_umum() {
			$list = $this->umum_model->get_all_umum();
	        $data = array();
	        $no = $_POST['start'];
	        foreach ($list as $field) {
	            $no++;
	            $row = array();
	            $row[] = $no;
	            $row[] = $field->tahun_anggaran;
	            $row[] = $field->nama_daerah;
	            $row[] = '<center>
		 	    	<a href="'.base_url("edit-data-umum?thn=").$field->tahun_anggaran.'" class="btn btn-info btn-flat btn-xs edit" data-thn="'.$field->tahun_anggaran.'" data-nama="'.$field->nama_daerah.'" data-nama="'.$field->username.'"  data-skpd="'.$field->id_skpd.'" data-role="'.$field->group_id.'"><i class="fa fa-pencil-square-o" aria-hidden="true" data-toggle="tooltip" title="Edit"></i></a>
		 	    	<a href="#" class="dropdown-item btn btn-danger btn-flat btn-xs hapus-data" data-id="'.$field->tahun_anggaran.'" data-nama="'.$field->nama_daerah.'" data-toggle="tooltip" title="Hapus"><i class="fa fa-trash-o" aria-hidden="true"></i></a></td>
		 	    	</center>';
	            $data[] = $row;
	        }
	 
	        $output = array(
	            "draw" => $_POST['draw'],
	            "recordsTotal" => $this->umum_model->count_all_umum(),
	            "recordsFiltered" => $this->umum_model->count_filtered_umum(),
	            "data" => $data,
	        );
	        echo json_encode($output);
		}

		public function get_combo_jenis_user(){

			echo $this->user_model->getjenisuser();	
		}

		public function get_combo_instansi(){
			echo $this->user_model->getinstansi();	
		}

		public function get_combo_prov(){
			echo $this->user_model->getprov();	
		}

		public function get_combo_kab(){
			$prov = $this->input->post('kode');
			
			echo $this->user_model->getkab($prov);	
		}

		public function getmax(){

			 $header = $this->input->post('kode');
			 $role = $this->input->post('role');
			 $username=$this->user_model->get_username($header,$role);
			 echo $username;
		}
		
		public function add(){

			if($this->input->post('submit')){

				$this->form_validation->set_rules('role', 'Role', 'trim|required');	
				$this->form_validation->set_rules('username', 'Username', 'trim|required');
				$this->form_validation->set_rules('password', 'Password', 'trim|required');
				$this->form_validation->set_rules('nama', 'Nama', 'trim|required');
				$this->form_validation->set_rules('email', 'Email', 'trim|required');

				$role = $this->input->post('role');
				if ($role == 2) {
					$this->form_validation->set_rules('prov', 'Provinsi', 'trim|required');
					$this->form_validation->set_rules('kab', 'Kabupaten', 'trim|required');			
				}
				

				if ($this->form_validation->run() == FALSE) {
					$data['view'] = 'setting/user/user_list';
					$this->load->view('template/layout', $data);
				}
				else{
					if ($role == 1) {
						$data = array(
							'name' => $this->input->post('nama'),
							'username' => $this->input->post('username'),
							'password' =>  password_hash($this->input->post('password'), PASSWORD_BCRYPT),
							'telp' => $this->input->post('telp'),
							'email' => $this->input->post('email'),
							'is_admin' => $role,
							'id_prov' => $role,
							'id_kab' => $role,
							'type' => $role,
							'id_group' => $role,
							'create_at' => date('Y-m-d H:i:s'),
							'update_at' => date('Y-m-d H:i:s'),
						);
					}else{
						$data = array(
							'name' => $this->input->post('nama'),
							'username' => $this->input->post('username'),
							'password' =>  password_hash($this->input->post('password'), PASSWORD_BCRYPT),
							'telp' => $this->input->post('telp'),
							'email' => $this->input->post('email'),
							'is_admin' => $role,
							'id_prov' => $this->input->post('prov'),
							'id_kab' => $this->input->post('kab'),
							'type' => $role,
							'id_group' => $role,
							'create_at' => date('Y-m-d H:i:s'),
							'update_at' => date('Y-m-d H:i:s'),
						);
					}
					$data = $this->security->xss_clean($data);
					$result = $this->user_model->add_user($data);
					if($result){
						$this->session->set_flashdata('msg', 'Data Berhasil Ditambahkan!');
						redirect(base_url('data-user'));
					}
				}
			}
			else{
				$data['view'] = 'setting/user/user_list';
				$this->load->view('template/layout', $data);
			}
			
		}

		public function insert(){
			$role = $this->input->post('role');



				
					if ($role == 1) {
						$data = array(
							'name' => $this->input->post('nama'),
							'username' => $this->input->post('username'),
							'password' =>  password_hash($this->input->post('password'), PASSWORD_BCRYPT),
							'telp' => $this->input->post('telp'),
							'email' => $this->input->post('email'),
							'is_admin' => $role,
							'id_prov' => $role,
							'id_kab' => $role,
							'type' => $role,
							'id_group' => $role,
							'create_at' => date('Y-m-d H:i:s'),
							'update_at' => date('Y-m-d H:i:s'),
						);
					}else if ($role == 2) {
						$tipe = $this->input->post('tipe_daerah');
						
						if ($tipe == 'prov') {
							$prov = $this->input->post('prov');
							$kab = $this->input->post('prov');
						}else{
							$prov = $this->input->post('prov');
							$kab = $this->input->post('kab');
						}
						$data = array(
							'name' => $this->input->post('nama'),
							'username' => $this->input->post('username'),
							'password' =>  password_hash($this->input->post('password'), PASSWORD_BCRYPT),
							'telp' => $this->input->post('telp'),
							'email' => $this->input->post('email'),
							'is_admin' => $role,
							'id_prov' => $prov,
							'id_kab' => $kab,
							'type' => $role,
							'id_group' => $role,
							'create_at' => date('Y-m-d H:i:s'),
							'update_at' => date('Y-m-d H:i:s'),
						);
					}else{
						$data = array(
							'name' => $this->input->post('nama'),
							'username' => $this->input->post('username'),
							'password' =>  password_hash($this->input->post('password'), PASSWORD_BCRYPT),
							'telp' => $this->input->post('telp'),
							'email' => $this->input->post('email'),
							'is_admin' => $role,
							'id_prov' => $this->input->post('instansi'),
							'id_kab' => $this->input->post('instansi'),
							'type' => $role,
							'id_group' => $role,
							'create_at' => date('Y-m-d H:i:s'),
							'update_at' => date('Y-m-d H:i:s'),
						);
					}
					$data = $this->security->xss_clean($data);
					$result = $this->user_model->add_user($data);
					if($result){
						$dataRet['pesan'] = "Data Berhasil Ditambahkan!";
					}
					echo json_encode($dataRet);
			
			
		}


		public function update(){
			$whereUpdate = array('username' => $this->input->post('username'));
			$role = $this->input->post('role');
			$password = $this->input->post('password');
			if($password != ''){
				$dataPass = array('password' => $password);
			}

			
				
					if ($role == 1) {
						$data = array(
							'name' => $this->input->post('nama'),
							// 'username' => $this->input->post('username'),
							// 'password' =>  password_hash($this->input->post('password'), PASSWORD_BCRYPT),
							'telp' => $this->input->post('telp'),
							'email' => $this->input->post('email'),
							// 'is_admin' => $role,
							// 'id_prov' => $role,
							// 'id_kab' => $role,
							// 'type' => $role,
							// 'id_group' => $role,
							// 'create_at' => date('Y-m-d H:i:s'),
							'update_at' => date('Y-m-d H:i:s'),
						);
					}else if ($role == 2) {
						$tipe = $this->input->post('tipe_daerah');
						
						if ($tipe == 'prov') {
							$prov = $this->input->post('prov');
							$kab = $this->input->post('prov');
						}else{
							$prov = $this->input->post('prov');
							$kab = $this->input->post('kab');
						}
						$data = array(
							'name' => $this->input->post('nama'),
							// 'username' => $this->input->post('username'),
							// 'password' =>  password_hash($this->input->post('password'), PASSWORD_BCRYPT),
							'telp' => $this->input->post('telp'),
							'email' => $this->input->post('email'),
							// 'is_admin' => $role,
							// 'id_prov' => $prov,
							// 'id_kab' => $kab,
							// 'type' => $role,
							// 'id_group' => $role,
							// 'create_at' => date('Y-m-d H:i:s'),
							'update_at' => date('Y-m-d H:i:s'),
						);
					}else{
						$data = array(
							'name' => $this->input->post('nama'),
							// 'username' => $this->input->post('username'),
							// 'password' =>  password_hash($this->input->post('password'), PASSWORD_BCRYPT),
							'telp' => $this->input->post('telp'),
							'email' => $this->input->post('email'),
							// 'is_admin' => $role,
							// 'id_prov' => $this->input->post('instansi'),
							// 'id_kab' => $this->input->post('instansi'),
							// 'type' => $role,
							// 'id_group' => $role,
							// 'create_at' => date('Y-m-d H:i:s'),
							'update_at' => date('Y-m-d H:i:s'),
						);
					}

					if($password != ''){
						$dataUpdate = array_merge($data,$dataPass);
					}else{
						$dataUpdate = $data;
					}
					$data = $this->security->xss_clean($dataUpdate);
					$result = $this->user_model->edit_user($data,$whereUpdate);
					if($result){
						$dataRet['pesan'] = "Data Berhasil DiUpdate!";
					}
					echo json_encode($dataRet);
			
			
		}


		public function edit($id = 0){
			
			if($this->input->post('submit')){
				$this->form_validation->set_rules('nama', 'Nama', 'trim|required');
				

				if ($this->form_validation->run() == FALSE) {
					$data['view'] = 'setting/users/user_list';
					$this->load->view('template/layout', $data);
				}
				else{
					
						$data = array(
								'name' => $this->input->post('nama'),
								'telp' => $this->input->post('telp'),
								'email' => $this->input->post('email'),
								'update_at' => date('Y-m-d H:i:s'),
						);

					if(!empty($this->input->post('password'))){
						$data['password'] = password_hash($this->input->post('password'), PASSWORD_BCRYPT);
					}
					$data = $this->security->xss_clean($data);
					$result = $this->user_model->edit_user($data, $id);
					if($result){
						$this->session->set_flashdata('msg', 'Data Berhasil Diupdate!');
						redirect(base_url('data-user'));
					}
				}
			}
			else{
				$data['view'] = 'setting/users/user_list';
				$this->load->view('template/layout', $data);
			}
		}



		public function del($id = 0){
			$this->db->delete('ms_user', array('id_user' => $id));
			$this->session->set_flashdata('msg', 'Data Berhasil Dihapus!');
			redirect(base_url('data-user'));
		}

	}


?>