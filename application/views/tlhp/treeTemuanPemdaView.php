...
<?php 
  $sql1 = "(SELECT noreg,no_lhp,id_aspek as kode,(SELECT nm_parameter from ms_was_umum where MID(id_parameter,2) = MID(tem.id_aspek,2) AND tahun = tem.tahun) as uraian, '' as nilai,tahun,'' as tlanjut,'' as sts_tlanjut, '' as file 
          FROM trd_tlhp_pemda_temuan tem where noreg = '".$noreg."' and no_lhp = '".$no_lhp."' AND LEFT(id_aspek,1) = 'U' GROUP BY id_aspek)
          UNION ALL
          (SELECT noreg,no_lhp,id_aspek as kode,(SELECT nm_parameter from ms_was_teknis where MID(id_parameter,2) = MID(tem.id_aspek,2) AND tahun = tem.tahun) as uraian, '' as nilai,tahun,'' as tlanjut,'' as sts_tlanjut, '' as file 
...
?>
        <li class="treeview-colorful-items"  style= "font-size:12pt;">
...
<?php 
    foreach ($res2 as $value2) {
      ...
      $sql3 = "(SELECT noreg,no_lhp,kd_rekom as kode, rekomendasi as uraian, '' as nilai, tahun,tlanjut,sts_tlanjut,file 
      ...
      <div class="modal-body">  
        ...
        <!-- <?php echo form_open('#', 'class="form-horizontal" id="form-input"');  ?>  -->
              <form class="form-horizontal" id="form-tl" enctype="multipart/form-data" method="post">
                  ...
                  <div class="col-sm-9">
                  <textarea rows='4' data-min-rows='4' name="rekomendasi" class="form-control input-sm" id="rekomendasi" maxlength="300" placeholder="" value="" readonly=""></textarea>
                  </div>
                  </div>
              ...
              <form class="form-horizontal" id="form-temuan" enctype="multipart/form-data" method="post">
                ...
                <div class="form-group row">
                 ...
                 <div class="form-group row">
                  ...
                  <div class="col-sm-3">
                    <input style = "color:darkgreen;" type="text" name="total_setor" class="form-control input-sm function_separator" id="total_setor" placeholder="" readonly="">
                  </div>
                </div>
                <div class="form-group row">
                  <label for="tlanjut" class="col-sm-2 control-label input-sm">Sisa Setoran</label>
                  <div class="col-sm-3">
                    <input style = "color:red;" type="text" name="sisa_temuan" class="form-control input-sm function_separator" id="sisa_temuan" placeholder="" readonly="">
                  </div>
                </div>
                              ...
                              <tfoot>
                                ...
                                <tr>
                                    ...
                                    <td>
                                      ...
                                      <!-- <input type="text" name="total_setor" id="total_setor" class="form-control input-sm function_separator " placeholder=""> -->
                                    </td>
                                    <td>
...
<script type="text/javascript">
  ...
  $('#form-tl').submit(function(e) {
        ...
        if (ev == 'justFile') {
            link = '<?php echo base_url('tlhp-pemda/insert-lampiran-tl'); ?>';
            ...
            link  = '<?php echo base_url('tlhp-pemda/insert-tindak-lanjut'); ?>';
        }
   ...
   $('#form-temuan').submit(function(e) {
        ...
        if (ev == 'justFile') {
            ...
            if (bukti == '' || setor == '' || tgl_setor == '' || catatan == '' ) {
              ...
              $.ajax({
                method: 'POST',
                url: '<?php echo base_url('tlhp-pemda/insert-setoran'); ?>',
                data: new FormData(this),
                contentType:false,
                cache : false,
                processData:false,
          ...
          var no_lhp    = $(this).attr("data-lhp");
           ...
           Swal.fire({
              ...
              if (result.value) {
                   ...
                   $.ajax({
                  method: 'POST',
                  url: '<?php echo base_url('tlhp-pemda/del-lampiran'); ?>?noreg='+noreg+'&lhp='+no_lhp+'&temuan='+kd_temuan+'&lamp='+kd_lamp,
                })
                .done(function(data) {
          ...
          var no_lhp    = $(this).attr("data-lhp");
           ...
           Swal.fire({
              ...
              if (result.value) {
                   ...
                   $.ajax({
                  method: 'POST',
                  url: '<?php echo base_url('tlhp-pemda/del-lampiran-tl'); ?>?noreg='+noreg+'&lhp='+no_lhp+'&rek='+kd_rek+'&lamp='+kd_lamp,
                })
                .done(function(data) {
  ...
  $(document).on("click", ".vTindak", function() {
   ...
   // $(document).on("click", "#upload_setor", function(e) {
          

   //        var noreg     = $('#noreg_temuan').val();
   //        var kode      = $('#kd_temuan').val();
   //        var no_lhp    = $('#lhp_temuan').val();

   //        var bukti     = $('#bukti_setor').val();
   //        var nilai     = $('#nilai_setor').val();
   //        var tgl       = $('#tgl_setor').val();
          
   //        var file = document.getElementById('nm_lampiran2');
          
   //          $.ajax({
   //        url: '<?php echo base_url('tlhp-pemda/insert-setoran'); ?>',
   //        type: 'POST',
   //        data:{noreg:noreg,no_lhp:no_lhp,kode:kode,bukti:bukti,nilai:nilai,tgl:tgl,file:file},
   //        contentType:false,
   //        cache : false,
   //        processData:false,
   //    }).done(function(data2) {
...
// console.log(objFileControl.files[0].type); 
    ...
    $(document).on("click", ".vSetoran", function() {
          ...
          $.ajax({
          url: '<?php echo base_url('tlhp-pemda/get-modal-setoran'); ?>',
          type: 'POST',
          data:{noreg:noreg,no_lhp:no_lhp,kode:kode}
      }).done(function(data2) {
