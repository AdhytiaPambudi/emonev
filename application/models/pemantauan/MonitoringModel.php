	...
	class MonitoringModel extends CI_Model{
		...
		public function viewPantauSKPD()
		{
			...
			foreach ($data as $value) {
				$no++;
				$kd_skpd 	= '<a href="'.base_url("monitoring-detail?skpd=".$value->kd_skpd).'">'.$value->kd_skpd.'</a>';
				$nm_skpd 	= '<a href="'.base_url("monitoring-detail?skpd=".$value->kd_skpd).'">'.$value->nm_skpd.'</a>';
		...
		public function viewPantauProgram($skpd)
		{
			...
			foreach ($data as $value) {
				$no++;
				$kd_program 	= '<a href="'.base_url("monitoring-detail-kegiatan?skpd=".$skpd).'&prog='.$value->kd_program.'">'.$value->kd_program.'</a>';
				$nm_program 	= '<a href="'.base_url("monitoring-detail-kegiatan?skpd=".$skpd).'&prog='.$value->kd_program.'">'.$value->nm_program.'</a>';
				$susun 		= number_format($value->susun,2,',','.');
				$ubah 		= number_format($value->ubah,2,',','.');
				$html.='<tr>
							<td style="width:5%;text-align:center;">'.$no.'</td>
		...
		public function viewPantauKegiatan($skpd,$prog)
		{
			...
			foreach ($data as $value) {
				$no++;
				$kd_kegiatan 	= '<a href="'.base_url("monitoring-rekening-kegiatan?skpd=".$skpd).'&prog='.$prog.'&keg='.$value->kd_kegiatan.'">'.$value->kd_kegiatan.'</a>';
				$nm_kegiatan 	= '<a href="'.base_url("monitoring-rekening-kegiatan?skpd=".$skpd).'&prog='.$prog.'&keg='.$value->kd_kegiatan.'">'.$value->nm_kegiatan.'</a>';
				$susun 		= number_format($value->susun,2,',','.');
				$ubah 		= number_format($value->ubah,2,',','.');
				$dok = $value->dok;
				if($dok == 0){
		...
		public function modalPantauRincianKegiatan($ta,$keg,$rek,$po)
		{
			...
			$query="SELECT a.*,b.bentuk,b.nilai_kontrak,b.kontraktor,b.no_kontrak,b.distrik,b.kampung,b.koordinat,b.keterangan,b.file_rab,b.file_kontrak,b.monitoring_meja,b.monitoring_lapangan,
		...
		public function modalDokumentasi($ta,$keg)
		{
			// $ta = $this->session->userdata('thn_ang');
			
			$query="SELECT a.*,p.uraian FROM (SELECT l.tahun_anggaran,l.kd_kegiatan,l.kd_rek,l.no_po,l.file,k.no_kontrak,k.kontraktor
			,r.distrik,r.kampung,r.koordinat  FROM trdreal_lamp l 
LEFT JOIN trdreal r ON l.tahun_anggaran = r.tahun_anggaran AND l.kd_kegiatan = r.kd_kegiatan AND l.kd_rek = r.kd_rekening AND l.no_po = r.no_rinci
LEFT JOIN trdreal_kontrak k ON l.tahun_anggaran = k.tahun_anggaran AND l.kd_kegiatan = k.kd_kegiatan AND l.kd_rek = k.kd_rek AND l.no_po = k.no_po
where l.`status` = 1 AND l.kd_kegiatan = '$keg' AND l.tahun_anggaran = $ta) a
LEFT JOIN trdpo p ON p.tahun_anggaran = a.tahun_anggaran AND p.kd_kegiatan = a.kd_kegiatan AND p.kd_rek5 = a.kd_rek AND p.no_po = a.no_po
;";
