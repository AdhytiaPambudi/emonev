<?php
	class BidangModel extends CI_Model{
		...
		var $table = 'm_bidang a'; //nama tabel dari database
	    var $column_order = array(null,'id'); //field yang ada di table 
	    var $column_search = array('nama','alias'); //field yang diizin untuk pencarian 
		...
		private function _get_all_bidang_query()
	    {
	    ...
	    public function get_bidang_by($id)
	    {
	    	$this->db->where('id',$id);
	        $this->db->from($this->table);
		...
		public function edit_bidang($data, $where){
			
			$this->db->where($where);
			$this->db->update('m_bidang', $data);
