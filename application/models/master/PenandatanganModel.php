<?php
	class PenandatanganModel extends CI_Model{

		var $table = 'm_ttd a'; //nama tabel dari database
	    var $column_order = array(null,'nip'); //field yang ada di table 
	    var $column_search = array('nama','jabatan'); //field yang diizin untuk pencarian 
		...
		private function _get_all_ttd_query()
	    {
            ...
            if($akses == 1){
				...
				$whereSKPD = array();
				$whereSKPD =explode(',', str_replace("'", "", $arrSKPD));
				$this->db->where_in('id_skpd', $whereSKPD);
		...
		public function edit_ttd($data, $where){
			$this->db->where($where);
