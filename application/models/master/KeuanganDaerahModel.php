<?php
	class KeuanganDaerahModel extends CI_Model{

		var $table = 'ms_indikator_keuangan a'; //nama tabel dari database
		...
		public function add_parameter($data){
			$this->db->insert('ms_indikator_keuangan', $data);
			return true;
		}
		...
		public function get_tree_parameter()
		{
			...
			$sql1 = 'SELECT * FROM ms_indikator_keuangan WHERE LENGTH(id_parameter) = 3 and tahun = '.$thn;
			...
			if (count($res1)>0) {
				...
				foreach ($res1 as $value1) {
					$html .= '<div class="panel panel-default panel-headline-custom">
						  ...
						  ';	
					$sql2 = "SELECT * FROM ms_indikator_keuangan WHERE LENGTH(id_parameter) = 5 and hd_parameter = '".$value1['id_parameter']."' and tahun = ".$thn;
					...
					if (count($res2)>0) {
						$html .= '<div id="collapse'.$value1["id_parameter"].'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading'.$value1["id_parameter"].'">
						  ...
						  ';
						  	$sql3 = "SELECT * FROM ms_indikator_keuangan WHERE LENGTH(id_parameter) = 7 and hd_parameter = '".$value2['id_parameter']."' and tahun = ".$thn;
		...
		public function get_parameter_mdb()
		{
			...
			if (count($res1)>0) {
				...
				foreach ($res1 as $value1) {
					...
					$sql2 = 'SELECT * FROM ms_indikator_keuangan WHERE LENGTH(id_parameter) = 5 and hd_parameter="'.$id_par1.'" and tahun = '.$thn;
					$res2 = $this->db->query($sql2)->result_array();
		...
		public function get_max_parameter($lvl,$header = '',$thn){
			...
			$this->db->select('max(SUBSTRING(id_parameter, 2)) AS id_parameter');
			...
			$this->db->from('ms_indikator_keuangan');
			$query = $this->db->get();
			...
			if ($result == 0 || $result == '') {
				...
				$max = $header.substr('0'.($result+1), -2);
			}
		...
		public function edit_parameter($data, $id, $thn){
			...
			$this->db->where('tahun', $thn);
			$this->db->update('ms_indikator_keuangan', $data);
			return true;
		}
