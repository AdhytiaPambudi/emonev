<?php
	class Pendapatan_model2 extends CI_Model{
		...
		function dashboard_real($tahun,$rekening){
			...
			$q = "SELECT ifnull(MAX(last_execute),'')tglbud FROM log_tarik_data WHERE table_name='trhkasin_ppkd'";
			$query = $this->DB2->query($q);
			$resultbud = $query->result_array();
			$tgl_bud = $resultbud[0]['tglbud'];
			
			$q = "SELECT ifnull(MAX(last_execute),'')tglopd FROM log_tarik_data WHERE table_name='sts_detail'";
			$query = $this->DB2->query($q);
			$resultbud = $query->result_array();
			$tgl_opd = $resultbud[0]['tglopd'];			
...
ON a.kd_rek=b.map_real) adx where $where2 group by kode");
			...
			if($kd_rek=='4.1.2.02'){  
				$this->db =  $this->load->database('pdam', TRUE);
				$query = "SELECT SUM(retribusi) AS realisasi FROM hl_lpp_coba 
			...
			$dataChart['tglmaxdataBUD'] = $this->tgl_ind($tgl_bud);
			$dataChart['tglmaxdataOPD'] = $this->tgl_ind($tgl_opd);
			return $dataChart;
		...
		function  tgl_ind($tgl){
			...
			$bulan  = $tanggal[1];
			$tahun  =  $tanggal[0];
			return   substr($tanggal[2],0,2).'-'.$bulan.'-'.$tahun.' '.substr($tanggal[2],3,9);
...
FROM trhkasin_ppkd a INNER JOIN trdkasin_ppkd b ON a.no_kas=b.no_kas AND a.no_sts=b.no_sts AND a.kd_skpd=b.kd_skpd
WHERE LEFT(b.kd_rek5,1) IN ('4') GROUP BY LEFT(b.kd_rek5,7))b
ON a.kd_rek=b.map_real) adx where $where2 ");
