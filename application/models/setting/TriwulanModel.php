<?php
	class TriwulanModel extends CI_Model{
		...
		public function viewTriwulan()
		{
			...
			if($akses == 1){
				$query="SELECT a.kd_skpd,a.nm_skpd,b.tahun_anggaran,b.show_triwulan1,b.show_triwulan2,b.show_triwulan3,b.show_triwulan4 FROM ms_skpd a
						LEFT JOIN 
						ms_config b on a.tahun_anggaran = b.tahun_anggaran AND a.kd_skpd = b.kd_skpd
						WHERE a.tahun_anggaran = $ta;";
			}else if($akses == 3){
				...
				$query="SELECT a.kd_skpd,a.nm_skpd,b.tahun_anggaran,b.show_triwulan1,b.show_triwulan2,b.show_triwulan3,b.show_triwulan4 FROM ms_skpd a
						LEFT JOIN 
						ms_config b on a.tahun_anggaran = b.tahun_anggaran AND a.kd_skpd = b.kd_skpd
						WHERE a.tahun_anggaran = $ta and a.kd_skpd in (".$where.");";
			}
			...
			foreach ($data as $value) {
				...
				$triw1 	= $value->show_triwulan1;
				$triw2 	= $value->show_triwulan2;
				$triw3 	= $value->show_triwulan3;
				$triw4 	= $value->show_triwulan4;
				$checkTriw1 = ($triw1=='Y') ? " checked " : "";
				$checkTriw2 = ($triw2=='Y') ? " checked " : "";
				$checkTriw3 = ($triw3=='Y') ? " checked " : "";
				$checkTriw4 = ($triw4=='Y') ? " checked " : "";
				

				
				$html.='<tr>
							<td style="width:5%;text-align:center;">'.$no.'</td>
							<td style="width:35%;text-align:left;">'.$nm_skpd.'</td>
							<td style="width:15%;text-align:center;"><input type="checkbox" '.$checkTriw1.' class="i-checks changeCheck" data-skpd="'.$kd_skpd.'" data-thn="'.$ta.'" data-tw="show_triwulan1"></td>
							<td style="width:15%;text-align:center;"><input type="checkbox" '.$checkTriw2.' class="i-checks changeCheck" data-skpd="'.$kd_skpd.'" data-thn="'.$ta.'" data-tw="show_triwulan2"></td>
							<td style="width:15%;text-align:center;"><input type="checkbox" '.$checkTriw3.' class="i-checks changeCheck" data-skpd="'.$kd_skpd.'" data-thn="'.$ta.'" data-tw="show_triwulan3"></td>
							<td style="width:15%;text-align:center;"><input type="checkbox" '.$checkTriw4.' class="i-checks changeCheck" data-skpd="'.$kd_skpd.'" data-thn="'.$ta.'" data-tw="show_triwulan4"></td>
						</tr>';
			}
		...
		(SELECT sum(Nilai_Ubah) FROM trdrka where kd_kegiatan = '".$keg."' and tahun_anggaran = '".$ta."') as tot_target_keg_ubah
			...
			foreach ($data as $value) {
				if($value->no== 0){
					...
					$uraian 	= '<a href="#" class="showModal" data-tahun="'.$ta.'" data-keg="'.$keg.'" data-rek="'.$value->kode.'" data-po="'.$value->no.'">'.$value->uraian.'</a>';
				}
				if($sts_ubah == 'Murni'){
		...
		public function modalPantauRincianKegiatan($ta,$keg,$rek,$po)
		{
			...
			$query="SELECT a.*,b.bentuk,b.nilai_kontrak,b.kontraktor,b.no_kontrak,b.distrik,b.kampung,b.koordinat,
		...
		(SELECT sum(Nilai) FROM trdrka where kd_kegiatan = '".$keg."' and tahun_anggaran = '".$ta."') as tot_target_keg,
		(SELECT sum(Nilai_Ubah) FROM trdrka where kd_kegiatan = '".$keg."' and tahun_anggaran = '".$ta."') as tot_target_keg_ubah,
		bentuk,nilai_kontrak,kontraktor,no_kontrak,distrik,kampung,koordinat,keterangan
			 FROM (
			 SELECT tahun_anggaran,kd_kegiatan,kd_rek5 as kode,uraian, no_po as no,
			 tvolume,tvolume_ubah,satuan1, satuan_ubah1, harga1,harga_ubah1,total,total_ubah FROM trdpo
		...
		public function add_triwulan($data){
		...
		public function edit_triwulan($data, $where){
			$this->db->where($where);
			$this->db->update('ms_config', $data);
			return true;
		}
