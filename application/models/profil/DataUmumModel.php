<?php
	class DataUmumModel extends CI_Model{

		

		// fungsi
		var $table_umum = 'ms_data_umum'; //nama tabel dari database
	    var $column_order_umum = array(null,'tahun_anggaran'); //field yang ada di table 
	    var $column_search_umum = array('tahun_anggaran','nm_daerah'); //field yang diizin untuk pencarian 
	    var $order_umum = array('tahun_anggaran' => 'asc'); // default order 


	    // data umum

	    public function add_umum($data){
			
			$this->db->insert($this->table_umum, $data);
			return true;
		}

		private function _get_all_umum_query()
	    {
	        ...
	        foreach ($this->column_search_umum as $item) // looping awal
	        {
	            if($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
	            {
	                ...
	                if(count($this->column_search_umum) - 1 == $i) 
	                    $this->db->group_end(); 
	            }
	            $i++;
	        }
	        ...
	        {
	            $order = $this->order_umum;
	            $this->db->order_by(key($order), $order[key($order)]);
	        }
	    }
	 
	    function get_all_umum()
	    {
	        $this->_get_all_umum_query();
	        if($_POST['length'] != -1)
